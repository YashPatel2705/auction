...
const OUTPUT_CSV_PATH = path.resolve(__dirname, "../registrations_photo_links.csv");
const OUTPUT_JSON_PATH = path.resolve(
  __dirname,
  "../registrations_photo_links.json",
);
...
function buildPhotoUploadLink(registrationId) {
  return `${BASE_URL}/photo-upload?rid=${encodeURIComponent(registrationId)}`;
}
...
async function main() {
  ...
  const outRows = [["email", "photo_upload_link"]];
  const outJson = [];
  ...
  for (let i = 1; i < rows.length; i += 1) {
    ...
    const photoUploadLink = buildPhotoUploadLink(registrationId);
    outRows.push([email, photoUploadLink]);
    outJson.push({ email, photoUploadLink });
  }
