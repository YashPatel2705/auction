...
export default function AdminPage() {
  ...
  if (user?.app_metadata?.role === 'players_viewer') {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column',
        alignItems: 'center', justifyContent: 'center', gap: 16, background: '#060e1a' }}>
        <div style={{ fontSize: 52 }}>🔒</div>
        <div className="teko" style={{ fontSize: 26, letterSpacing: 2, color: '#ef4444' }}>ACCESS DENIED</div>
        <div style={{ fontSize: 13, color: '#5a8fba', fontWeight: 600 }}>You don't have permission to view this page.</div>
        <button onClick={signOut}
          style={{ marginTop: 8, background: 'none', border: '1px solid #1e3a5f', borderRadius: 8,
            color: '#ef4444', fontSize: 13, fontWeight: 700, padding: '8px 20px', cursor: 'pointer' }}>
          Sign out
        </button>
      </div>
    )
