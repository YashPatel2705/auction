    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 20 }}>
      ...
      <div style={{ display: 'flex', gap: 8 }}>
        <input type="password" placeholder="Enter admin password…" value={input}
          ...
          style={{ background: error ? '#3a0d0d' : '#0a1e35', border: `1px solid ${error ? '#ef4444' : '#1e3a5f'}`, borderRadius: 10, padding: '12px 18px', color: '#fff', fontSize: 15, width: 280, transition: 'all .2s' }} />
        <button onClick={attempt}
          style={{ background: 'linear-gradient(135deg,#00c864,#007a3d)', border: 'none', borderRadius: 10, padding: '12px 22px', color: '#fff', fontSize: 15, fontWeight: 700, cursor: 'pointer' }}>
          Enter
        </button>
      </div>
...
export default function AdminPage() {
  ...
  return (
    <div style={{ minHeight: '100vh' }}>
      ...
      {/* Nav */}
      <div style={{ background: '#0a1628', borderBottom: '1px solid #1e3a5f', padding: '0 20px', overflowX: 'auto' }}>
        <div style={{ maxWidth: 1440, margin: '0 auto', display: 'flex', minWidth: 'max-content' }}>
          {TABS.map(t => (
            <button key={t.id} onClick={() => setView(t.id)}
              style={{
                background: 'none', border: 'none',
                borderBottom: view === t.id ? '2px solid #00c864' : '2px solid transparent',
                padding: '13px 16px', color: view === t.id ? '#00c864' : '#4a7fa8',
      ...
      <div style={{ maxWidth: 1440, margin: '0 auto', padding: 18 }}>
        {view === 'spin'    && <SpinWheel onTeamSelected={team => showToast(`${team.name} is up next!`)} />}

        {view === 'auction' && (
          <AuctionStage players={players} teams={teams} onSell={sellPlayer} showToast={showToast}
            jumpToPlayer={poolJump} onJumpConsumed={() => setPoolJump(null)} />
        )}
        ...
        {view === 'teams' && (
          <TeamsView players={players} teams={teams} onRelease={releasePlayer} isAdmin={true} showToast={showToast} />
        )}

        {view === 'manage-players' && (
          <ManagePlayers players={players} onUpdate={updatePlayer} onDelete={deletePlayer} onAdd={addPlayer} showToast={showToast} />
        )}

        {view === 'manage-teams' && (
          <ManageTeams teams={teams} onAdd={addTeam} onUpdate={updateTeam} onDelete={deleteTeam} showToast={showToast} />
        )}
      </div>
