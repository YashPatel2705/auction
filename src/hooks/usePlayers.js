...
export function usePlayers() {
  ...
  const sellPlayer = useCallback(async ({ playerId, teamId }) => {
    const { error } = await supabase
      .from('players').update({ status: 'sold', sold_to: teamId }).eq('id', playerId)
    if (error) throw new Error(error.message)
  }, [])

  const releasePlayer = useCallback(async (playerId) => {
    const { error } = await supabase
      .from('players').update({ status: 'available', sold_to: null }).eq('id', playerId)
    if (error) throw new Error(error.message)
  }, [])

  const resetAuction = useCallback(async () => {
    const { error } = await supabase
      .from('players').update({ status: 'available', sold_to: null }).neq('id', 0)
    if (error) throw new Error(error.message)
  }, [])

  const updatePlayer = useCallback(async (id, fields) => {
    const { error } = await supabase
      .from('players').update({ name: fields.name?.trim(), rating: fields.rating, role: fields.role }).eq('id', id)
    if (error) throw new Error(error.message)
  }, [])
  ...
  const addPlayer = useCallback(async ({ name, role, rating }) => {
    ...
    const { error } = await supabase
      .from('players').insert({ id: maxId + 1, name: name.trim(), role, rating: Number(rating), status: 'available' })
    if (error) throw new Error(error.message)
  }, [players])
